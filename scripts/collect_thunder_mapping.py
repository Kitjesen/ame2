...
parser.add_argument("--pose-reference", help="Replay saved poses and scan windows for a paired mount comparison")
...
def collect():
    ...
    reference = torch.load(args.pose_reference, map_location="cpu", weights_only=True) if args.pose_reference else None
    if reference is not None:
        splits = reference["tile_splits"]
    ...
    dataset["paired_native_label_boundary_cells"] = 0
    dataset["paired_native_label_max_difference_m"] = 0.
    ...
    for split_index, (split, tiles) in enumerate(splits.items()):
        values = {k: [] for k in ("raw", "clean_raw", "observed", "clean_observed", "truth", "tile", "counts", "pose", "pattern_start")}
        ...
        for batch in range(batches):
            ...
            root[:, 7:] = 0.
            batch_slice = slice(batch*b, (batch+1)*b)
            if reference is not None:
                root[:, :7] = reference[split]["pose"][batch_slice].to(args.device)
                tile = reference[split]["tile"][batch_slice].to(args.device)
                from isaaclab.utils.math import euler_xyz_from_quat
                yaw = euler_xyz_from_quat(root[:, 3:7])[2]
            ...
            scene.update(.1)
            if reference is not None:
                starts = reference[split]["pattern_start"][batch_slice].to(args.device)
                sensor._sequence.cursors[:] = starts
                body._sequence.cursors[:] = starts
            environment, self_hit = sensor.get_observation(), body.get_observation()
            starts = (sensor._sequence.cursors - config["samples_per_scan"]) % len(sensor._sequence.directions)
            ...
            noise_generator = torch.Generator(device=args.device).manual_seed(args.seed + split_index*10000 + batch)
            noisy_range = ranges + torch.randn(ranges.shape, device=args.device, generator=noise_generator) * .02
            ...
            noisy_valid &= torch.rand(ranges.shape, device=args.device, generator=noise_generator) >= .05
            ...
            if reference is not None:
                canonical = reference[split]["truth"][batch_slice].to(args.device)
                difference = (truth-canonical).abs()
                boundary = difference > 2e-4
                if boundary.any():
                    # Sub-millimetre ray rounding can choose opposite sides of a
                    # discontinuity. Only sparse, neighbour-matching differences
                    # qualify; broader pose/terrain mismatches still fail.
                    neighbours = torch.nn.functional.unfold(
                        torch.nn.functional.pad(canonical, (1,1,1,1), mode="replicate"), 3)
                    nearest = (neighbours - truth.flatten(2)).abs().min(1).values.reshape_as(truth)
                    assert (nearest[boundary] <= 2e-4).all(), "Paired terrain labels changed away from a boundary"
                    assert boundary.sum() <= truth.numel()*.001, "Paired terrain labels differ at too many cells"
                    dataset["paired_native_label_boundary_cells"] += int(boundary.sum())
                    dataset["paired_native_label_max_difference_m"] = max(
                        dataset["paired_native_label_max_difference_m"], difference.max().item())
                truth = canonical
                torch.testing.assert_close(starts.cpu(), reference[split]["pattern_start"][batch_slice])
            ...
            for key, value in (("raw", scan.height), ("observed", scan.observed),
                               ...
                               ("pose", root[:, :7]), ("pattern_start", starts)):
                values[key].append(value.cpu())
            if args.preview and split == "test" and batch == 0:
                from mid360_preview import snapshot
                from train_mid360_mapping import edges
                k = int(edges(truth).flatten(1).sum(-1).argmax())
                dataset["preview"] = snapshot(robot, sensor, body, points, valid & ~blocked, tf, poses,
                                                truth, scan.height, scan.observed, k)
    ...
    dataset["paired_pose_reference"] = args.pose_reference
    torch.save(dataset, output)
